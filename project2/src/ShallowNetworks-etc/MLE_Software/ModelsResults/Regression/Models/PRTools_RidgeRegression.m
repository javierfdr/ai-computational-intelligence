...
function Output = PRTools_RidgeRegression (traintest,DataSet,Model,Param);
...
if ~strcmp(traintest,'train') && ~strcmp(traintest,'test')
  error('PRTools_RidgeRegression: String "traintest" invalid');
end;
...
if strcmp(traintest,'train')   %%% Construct the model
  ...
  prwarning(2);
  Output = ridger(DataSet_PRTools,Lambda);
  prwarning off;
else                           %%% Test the model
