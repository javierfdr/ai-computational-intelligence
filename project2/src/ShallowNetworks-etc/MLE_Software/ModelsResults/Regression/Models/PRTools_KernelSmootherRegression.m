...
function Output = PRTools_KernelSmootherRegression (traintest,DataSet,Model,Param);
...
if ~strcmp(traintest,'train') && ~strcmp(traintest,'test')
  error('PRTools_KernelSmootherRegression: String "traintest" invalid');
end;
...
if strcmp(traintest,'train')   %%% Construct the model
  if isfield(Param,'Width') Width = Param.Width; else Width = 1; end;
  prwarning(2);
  Output = ksmoothr(DataSet_PRTools,Width);
  prwarning off;
else                           %%% Test the model
